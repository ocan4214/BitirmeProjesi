    ...
    public class UserController : Controller
    {
        ...
        public ActionResult Registration([Bind(Exclude = "IsEmailVerified,ActivationCode")]User user)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                Profile userProfile = new Profile() { CreateDate =DateTime.Now , LastUpdateDate =DateTime.Now };           
                user.Profile = userProfile;
                user.Roles.Add(new Role() { RoleName = Enum.GetName(typeof(RoleEnum), RoleEnum.Member), RoleType = (int)RoleEnum.Member});
                #endregion
                ...
                {//Login password'u hash ile sha256 encrypt ettik daha sonra encrypt hallerini kıyaslıyoruz
                    if (string.Compare(Crypto.Hash(login.Password), user.Password) == 0)//0 ise birbirlerine eşit oluyorlar 0 dan küçük ise ilk string daha önce geliyor
                    {
                        if (user.IsEmailVerified)
                        {
                            ...
                            var ticket = new FormsAuthenticationTicket(user.UserId.ToString(), login.RememberMe, timeout);//İncele cookie oluşturuyor timeout dakika cinsinden
                            ...
                            {
                                return RedirectToAction("ProfilePage", "Profile", new { id = user.Profile.ProfileId });
                            }
                            else
                            {
                                return RedirectToAction("ProfilePage", "Profile",new { id=user.Profile.ProfileId  });
        ...
        public void SendVerificationLink(string Email, string activitionCode, string emailFor = "VerifyAccount")
        {
            ...
            var fromEmail = new MailAddress("BLABLAEMAİL", "BLABLA");
            var toEmail = new MailAddress(Email);
            var fromEmailPassword = "SİFREGİR";
            string subject = "";
            string body = "";
